...
export function listPlayersTool(api: FplApi) {
  return {
    ...
    execute: async ({ position, team, minXp, maxCost, limit }: {
      ...
      console.log('[list_players] Starting execution with params:', { position, team, minXp, maxCost, limit });
      
      try {
        console.log('[list_players] Calling api.players.playersList()...');
        const res = await api.players.playersList();
        console.log('[list_players] API response received:', { 
          success: !!res, 
          hasData: !!res?.data, 
        ...
        console.log('[list_players] Parsed players count:', players.length);
        ...
        console.log('[list_players] Filtered and capped to:', capped.length, 'players');
        ...
        console.log('[list_players] Returning result with', result.length, 'players');
        return result;
      } catch (error) {
        console.error('[list_players] Error calling FPL API:', error);
        throw error;
      }
    },
  } as const;
